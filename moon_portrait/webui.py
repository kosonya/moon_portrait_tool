...
MAP_VERSION_MARKER = "moon-portrait-map-version: v7"
...
def create_app(data_dir: Path, results_dir: Path) -> Flask:
    ...
    # Per-run mutex for concurrent /delete_pair. The optimistic JS fires
    # requests as fast as the user can click; we need to make sure two
    # writes to the same run don't race (last-writer-wins would lose
    # earlier deletes). One lock per run id is created lazily.
    _delete_locks_map: dict[str, threading.Lock] = {}
    _delete_locks_meta = threading.Lock()
    def _delete_lock_for(rid: str) -> threading.Lock:
        with _delete_locks_meta:
            lk = _delete_locks_map.get(rid)
            if lk is None:
                lk = threading.Lock()
                _delete_locks_map[rid] = lk
            return lk

    @app.route("/delete_pair/<run_id>", methods=["POST"])
    def delete_pair(run_id):
        """Permanently delete every candidate sharing the requested
        (camera, model) location bin from the run. CSV, GeoJSON and
        map.html are rewritten; meta.json summary updated.

        Serialized per-run-id so concurrent client requests can't race
        (client fires-and-forgets — multiple deletes may overlap)."""
        ...
        try:
            cam_x = float(body["cam_x"]); cam_y = float(body["cam_y"])
            mod_x = float(body["mod_x"]); mod_y = float(body["mod_y"])
        except (KeyError, TypeError, ValueError):
            return Response("missing or invalid coords", status=400)
        snap_m = float(body.get("snap_m", 75.0))
        if snap_m <= 0:
            return Response("snap_m must be > 0", status=400)

        with _delete_lock_for(run_id):
            csv_path = run_dir / "candidates.csv"
            if not csv_path.is_file():
                return Response("no candidates.csv", status=404)
            try:
                cands = _load_candidates_from_csv(csv_path)
            except Exception as e:  # noqa: BLE001
                log.exception("failed to load candidates for delete")
                return Response(f"failed to load CSV: {e}", status=500)

            bin_cam = (round(cam_x / snap_m), round(cam_y / snap_m))
            bin_mod = (round(mod_x / snap_m), round(mod_y / snap_m))
            def matches(c):
                return ((round(c.camera_x / snap_m),
                         round(c.camera_y / snap_m)) == bin_cam
                        and (round(c.model_x / snap_m),
                             round(c.model_y / snap_m)) == bin_mod)

            before = len(cands)
            kept = [c for c in cands if not matches(c)]
            n_deleted = before - len(kept)
            if n_deleted == 0:
                return Response("no matching pair", status=404)

            try:
                write_csv(kept, csv_path)
                write_geojson(kept, run_dir / "candidates.geojson")
                write_map(kept, run_dir / "map.html")
            except Exception as e:  # noqa: BLE001
                log.exception("failed to write outputs after delete")
                return Response(f"write failed: {e}", status=500)

            # Update meta.json summary counts so the sidebar reflects the
            # change after refresh.
            meta_path = run_dir / "meta.json"
            if meta_path.is_file():
                try:
                    meta = json.loads(meta_path.read_text())
                    pairs = cluster_by_pair(kept, snap_m=snap_m)
                    summary = meta.get("summary", {})
                    summary["n_unique"] = len(kept)
                    summary["n_pairs"] = len(pairs)
                    meta["summary"] = summary
                    meta_path.write_text(json.dumps(meta, indent=2))
                except Exception:  # noqa: BLE001
                    log.exception("failed to update meta.json")

            return jsonify(ok=True, n_deleted=n_deleted, remaining=len(kept))
